      ...
      <table id="table" class="table-auto rounded-sm shadow-md">
        ...
        <thead class="border-none bg-gss-dark text-white">
          ...
          <tr>
            ...
            <th class="py-4 px-2">Nama Role</th>
  ...
  export default {
    ...
    mounted: function() {
      ...
      app.datatable = $('#table').DataTable({
        ...
        pageLength: 10,
        ajax: {
          url: `${process.env.VUE_APP_API_BASE_URL}/${process.env.VUE_APP_API_ADMIN_SLUG}/role/datatable`,
          type: 'get',
          xhrFields: {
          ...
          dataSrc: function(json) {
            ...
            if (data.length > 0) {
              Array.prototype.forEach.call(data, function(item, i){
                var color = (item.rol_status == 'aktif') ? 'text-success-calm bg-success-light' : 'text-error-calm bg-error-light';
                data[i].status = item.rol_status;
                data[i].rol_status = `<span class="${color} alert font-bold">${item.rol_status}</span>`; 
                ...
                data[i].rol_created_at = `${date} - ${time}`;
              });
        ...
        columns: [
              ...
              { data: "rol_nama" },
              { data: "rol_status", className: 'admin-table-status capitalize' },
              { data: "rol_created_at", className: 'admin-table-time'}
        ],
